...
| Angular | Consulta, apresentação e, só via o botão manual condicional do detalhe, disparo sob demanda de `POST /validar` (ADR-0010) | Decidir validade ou aprovação; chamar `POST /validar` fora do botão condicional |
...
1. **Botão de validar existe, mas é condicional:** só aparece no detalhe quando `status` é `PENDENTE` ou `REPROVADA` (RC-15, ADR-0010); some quando `APROVADA`.
2. Fora desse botão, nenhum service/componente Angular chama `POST /validar` (nem listagem, nem carregamento normal do detalhe).
3. Zero lógica de validade no frontend — o botão só dispara a chamada e exibe a resposta; a decisão continua 100% no backend.
...
O Swagger continua sendo a superfície completa para demonstrar `POST /validar`; o Angular expõe uma fatia dele (disparo, sem parâmetros extras) pelo botão manual condicional.
...
- **botão "Validar agora" visível só em `PENDENTE`/`REPROVADA`, chamando `POST /validar` e recarregando o detalhe (ADR-0010);**
- **ausência de chamada a `POST /validar` em qualquer outro fluxo (listagem, carregamento normal do detalhe, `APROVADA`)**;
- histórico ilustrativo exibido para `APROVADA`, com a entrada fictícia marcada.
...
| V-15 | **Frontend só chama `POST /validar` pelo botão "Validar agora" do detalhe, visível apenas em `PENDENTE`/`REPROVADA` (ADR-0010); nenhum outro fluxo o chama** |
