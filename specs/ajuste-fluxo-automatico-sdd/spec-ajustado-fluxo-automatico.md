...
| RC-07 | Auditoria de validação é **persistida e append-only**. Não há endpoint próprio nem tela de histórico. |
...
| RC-15 | `POST /validar` permanece disponível para cumprir o contrato técnico do case, Swagger e testes, reutilizando o mesmo caso de uso de validação. **Não é chamado pelo Angular e não é o gatilho normal do produto.** |
...
| RF-15 | Frontend não possui ação de validar e não chama `POST /validar`; apresenta o estado produzido pelo backend | CA-018 |
...
Ele usa exatamente o mesmo caso de uso chamado pelo Worker. O Angular nunca o chama.
...
Mantido como **adaptador síncrono técnico** do caso de uso de validação. Não é usado pelo Angular.
...
| CA-018 | **Não existe botão/ação de validar no frontend e nenhum service/componente Angular chama `POST /validar`.** |
...
Endpoint público de criação de movimentação · tela/endpoint para aprovar ou reprovar · endpoint de histórico de auditoria · tela de histórico de auditoria · autenticação e ator · efetivação da movimentação em sistemas corporativos após aprovação · broker externo no MVP (SQS, RabbitMQ, Kafka etc.) · cache distribuído · severidade `AVISO` · E2E · teste de carga · versionamento temporal de entidades · implementação de `PX01–PX05` · demais itens de RC-11.

O `POST /validar` existe por exigência técnica do case, mas não constitui fluxo manual do frontend.
...
| R3 | Frontend volta a disparar validação | CA-018 + teste de service/componente |
