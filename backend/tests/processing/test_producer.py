...
def test_ca040_todas_aprovadas_cria_job_pendente(db_session):
...
def test_ca041_aprovacao_pendente_nao_cria_job(db_session):
    mov = _transferencia_valida(db_session)
...
def test_ca042_aprovacao_reprovada_bloqueia_sem_job(db_session):
    mov = _transferencia_valida(db_session)
...
def test_producer_nao_reprocessa_movimentacao_ja_bloqueada(db_session):
    ...
    # mov.status já é REPROVADA após a primeira execução, então não é mais
    # candidata (query filtra status=PENDENTE) — nenhuma ação na segunda.
